...
const Favourites = () => {
    ...
    catalogService
      ...
      .then((result) => {
        const filteredItems = result.filter(
          (item) => userFavs.includes(item._id), // Filter based on item _id being in userFavorites
        );
  ...
  return (
    <div className={styles.main}>
      {items.length > 0 ? (
        <>
          <div className={styles.cheekyText}>We know you love them...</div>
        ...
        <div className={styles.cheekyText}>No favs, how sad...</div>
      )}
    </div>
  );
};
